...
window.alternarTela = (cadastrar) => {
    ...
    document.getElementById("subtituloLogin").innerText = cadastrar ? "Crie sua conta gratuita agora" : "Seu controle financeiro inteligente";
};
...
window.excluirReceita = async (id) => { if(confirm("Excluir receita?")) await deleteDoc(doc(db, "recorrencias", id)); };
...
window.darBaixa = async (id) => { if(confirm("Confirmar pagamento?")) await deleteDoc(doc(db, "dividas", id)); };
...
window.perguntarIA = () => {
    ...
    let resp = "Não entendi. Tente perguntar sobre sua saúde, guardar dinheiro ou suas metas.";
    const saldo = totalReceitas - totalDividas;

    if(msg.includes("saúde") || msg.includes("situacao")) {
        const p = (totalDividas / totalReceitas) * 100;
        resp = `Sua saúde financeira está ${(p < 40) ? 'ÓTIMA' : 'EM ALERTA'}. Suas dívidas representam ${p.toFixed(1)}% do que você ganha.`;
    } else if(msg.includes("guardar") || msg.includes("reserva")) {
        resp = `Com saldo de ${BRL(saldo)}, recomendo guardar ${BRL(saldo * 0.15)} (15%) hoje!`;
    } else if(msg.includes("meta")) {
        resp = `Suas metas são: ${listaMetasNomes.join(", ")}. Vamos focar nelas?`;
    }
...
function carregarDados() {
  onSnapshot(query(collection(db, "recorrencias"), where("userId", "==", userUID)), snap => {
    totalReceitas = 0; document.getElementById("listaGanhos").innerHTML = "";
    snap.forEach(d => {
        ...
        document.getElementById("listaGanhos").innerHTML += `<li class="flex justify-between items-center p-4 glass rounded-2xl border-l-4 border-emerald-500">
            ...
            <button onclick="excluirReceita('${d.id}')" class="text-slate-600 hover:text-rose-500"><i class="fas fa-trash-alt"></i></button></li>`;
    });
    atualizarUI();
  });

  onSnapshot(query(collection(db, "dividas"), where("userId", "==", userUID)), snap => {
    totalDividas = 0; document.getElementById("listaDividasGrid").innerHTML = "";
    const hoje = new Date().toISOString().split('T')[0];
    snap.forEach(d => {
        const item = d.data(); totalDividas += item.valor; const eHoje = item.vencimento === hoje;
        document.getElementById("listaDividasGrid").innerHTML += `<div class="glass p-6 rounded-[2rem] border-2 ${eHoje ? 'border-rose-600 animate-pulse' : 'border-slate-800'}">
            <div class="flex justify-between items-start mb-4"><span class="text-[9px] font-black ${eHoje ? 'text-rose-500' : 'text-slate-500'}">VENCE: ${item.vencimento}</span>
                <button onclick="darBaixa('${d.id}')" class="text-rose-400 font-bold text-[10px]">PAGO</button></div>
            <p class="font-bold text-slate-300 text-sm italic">${item.nome}</p><p class="text-2xl font-black text-rose-400 italic">${BRL(item.valor)}</p></div>`;
    });
    atualizarUI();
  });

  onSnapshot(query(collection(db, "metas"), where("userId", "==", userUID)), snap => {
    document.getElementById("listaMetasGrid").innerHTML = ""; listaMetasNomes = [];
    snap.forEach(d => {
        const item = d.data(); listaMetasNomes.push(item.nome); const perc = Math.min(100, (item.atual / item.alvo) * 100).toFixed(0);
        document.getElementById("listaMetasGrid").innerHTML += `<div class="glass p-8 rounded-[2.5rem] border border-white/5">
            <div class="flex justify-between font-black text-sm mb-4 italic"><span>${item.nome}</span><span class="text-purple-400">${perc}%</span></div>
            ...
            <div class="flex gap-3"><button onclick="ajustarMeta('${d.id}', 'add')" class="flex-1 bg-emerald-500/10 text-emerald-400 p-4 rounded-2xl text-[11px] font-black">+ VALOR</button>
                ...
                <button onclick="deleteDoc(doc(db,'metas','${d.id}'))" class="p-4 text-slate-600"><i class="fas fa-trash-alt"></i></button></div></div>`;
    });
  });
...
function atualizarUI() {
    const s = totalReceitas - totalDividas;
    ...
    document.getElementById("resumoSaldo").innerText = BRL(s);
    const p = totalReceitas > 0 ? (totalDividas / totalReceitas) * 100 : 0;
    const bar = document.getElementById("barSaude"); const txt = document.getElementById("txtSaude");
